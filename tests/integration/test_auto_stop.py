...
Code: garuda_tunnel/_worker.py::_idle_watchdog,
      garuda_tunnel/activity.py::ActivityTracker.
"""
...
def test_idle_auto_stop_kills_daemon(
    ...
    # Lockfile cleaned up by graceful shutdown.
    lock_path = _state_dir() / f"{token}.lock"
    assert not lock_path.exists(), f"stale lockfile {lock_path}"
...
def test_active_connection_prevents_auto_stop(
    ...
    started_daemons.append((pid, body["token"]))
